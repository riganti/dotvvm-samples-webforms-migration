...
### Step 1: The `01_webforms` branch contains a simple Web Forms application with a few pages and a simple usage of the `GridView` control.
...
* Add the OWIN Startup Class [Startup.cs](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/02_install_dotvvm/src/OldWebApp/OldWebApp/Startup.cs).
* Add the DotVVM Configuration file [DotvvmStartup.cs](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/02_install_dotvvm/src/OldWebApp/OldWebApp/DotvvmStartup.cs).
...
* A master page [Site.dotmaster](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/03_migration_in_progress/src/OldWebApp/OldWebApp/Views/Site.dotmaster) was created. Its content was copied with a few changes made: `asp:ContentPlaceHolder` was changed to `dot:ContentPlaceHolder`, all the `runat="server"` were removed and so on. 
* A viewmodel [SiteViewModel.cs](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/03_migration_in_progress/src/OldWebApp/OldWebApp/ViewModels/SiteViewModel.cs) with an abstract property `Title` was added in the project. 
* A resources for jQuery and Bootstrap were configured in [DotvvmStartup.cs](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/03_migration_in_progress/src/OldWebApp/OldWebApp/DotvvmStartup.cs).
* The pages [About.dothtml](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/03_migration_in_progress/src/OldWebApp/OldWebApp/Views/About.dothtml) and [Contact.dothtml](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/03_migration_in_progress/src/OldWebApp/OldWebApp/Views/Contact.dothtml) were migrated in DotVVM.
* The route registrations for the migrated pages were moved from [Global.asax.cs](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/03_migration_in_progress/src/OldWebApp/OldWebApp/Global.asax.cs) to [DotvvmStartup.cs](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/03_migration_in_progress/src/OldWebApp/OldWebApp/DotvvmStartup.cs).
...
* The [Default.dothtml](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/04_migration_complete/src/OldWebApp/OldWebApp/Views/Default.dothtml) was migrated and demonstrates the [GridView](https://www.dotvvm.com/docs/controls/builtin/GridView/latest) and [DataPager](https://www.dotvvm.com/docs/controls/builtin/DataPager/latest) controls in DotVVM. 
* The [DefaultViewModel.cs](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/04_migration_complete/src/OldWebApp/OldWebApp/ViewModels/DefaultViewModel.cs) demonstrates the MVVM approach for data-binding and the usage of `GridViewDataSet`.

Now the application doesn't use any of the ASP.NET Web Forms features, so it's ready to be switched to .NET Core.
...
### Step 5: The `05_dotnetcore` branch shows the changes made to the project file in order to switch the project to .NET Core 2.2.

* The [OldWebApp.csproj](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/05_dotnetcore/src/OldWebApp/OldWebApp/OldWebApp.csproj) was converted to the new format and the `Dotvvm.Owin` package was replaced by `Dotvvm.AspNetCore`.
* The scripts and CSS files were moved to the `wwwroot` folder.
* The [Startup.cs](https://github.com/riganti/dotvvm-samples-webforms-migration/tree/05_dotnetcore/src/OldWebApp/OldWebApp/Startup.cs) was updated to use the ASP.NET Core `IServiceCollection` and `IAppBuilder`.
